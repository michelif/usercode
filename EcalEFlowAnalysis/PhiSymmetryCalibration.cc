...
//   variablesTree_endc_->Branch("eventid",&eventid,"eventid/i");  
  ...
  */
  ...
  ostringstream tree_name_endc;   
  tree_name_endc<<"kFactorTree_endc";
  kFactorFile->cd();
  kFactorTree_endc=new TTree(tree_name_endc.str().c_str(),tree_name_endc.str().c_str());	      
  //  cout<<"TTree created"<<endl;						      
  kFactorTree_endc->Branch("miscal",&miscal_var_endc,"miscal/F");  
  kFactorTree_endc->Branch("etsum",&etsum_var_endc,"etsum/F");	
  kFactorTree_endc->Branch("ix",&ix_var_endc,"ix/I");
  kFactorTree_endc->Branch("iy",&iy_var_endc,"iy/I");
  kFactorTree_endc->Branch("sign",&sign_var_endc,"sign/I");
  ...
  for (int imiscal=0; imiscal<kNMiscalBinsEE; imiscal++) {
    ...
    for (int ix=0; ix<kEndcWedgesX; ix++) {
      for (int iy=0; iy<kEndcWedgesY; iy++) {
	for(int isides=0;isides<kSides;isides++){
	etsum_endc_miscal_[imiscal][ix][iy][isides]=0.;
	}
      }
    }
...
//    cout<<"et"<<et<<"energy"<<e<<"eta"<<eta<<"phi"<<phi<<endl;  
  ...
  for (ite=endcapRecHitsHandle->begin(); ite!=endcapRecHitsHandle->end(); ite++) {
	...
	if (miscalEE_[imiscal]*e> eCut_endc && et*miscalEE_[imiscal] < et_thr && e_.goodCell_endc[hit.ix()-1][hit.iy()-1][sign]){
	  etsum_endc_miscal_[imiscal][hit.ix()-1][hit.iy()-1][sign] += miscalEE_[imiscal]*et;
...
void PhiSymmetryCalibration::getKfactors()
{
  ...
  for (int ix=0; ix<kEndcWedgesX; ix++) {
	...
	for (int imiscal=0; imiscal<kNMiscalBinsEE; imiscal++) {
	  ...
	  if(etsum_endc_miscal_[middlebin][ix][iy][isides]!=0) {
	    miscal_var_endc=miscalEE_[imiscal]-1.;
	    etsum_var_endc=etsum_endc_miscal_[imiscal][ix][iy][isides];
	    ix_var_endc=ix;
	    iy_var_endc=iy;
	    sign_var_endc=isides;
	  }
	  
	  if(miscal_var_endc!=-1 || etsum_var_endc != -1 ) kFactorTree_endc->Fill();
