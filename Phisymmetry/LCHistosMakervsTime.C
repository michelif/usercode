...
if (!LC_barl_in.is_open()) {
  ...
  nameFileStream<<"LCHistosvsTime_"<<iFile<<".root";
  ...
  for(int iieta=0;iieta<kBarlRings;iieta++){
	...
	dummyStream<<"LCvsEta_"<<iFile<<"_"<<iieta+1<<"_"<<iiphi+1<<"_"<<iisign;
	...
	graphLCvsTime[iieta][iiphi][iisign]->SetMarkerSize(1.);
	graphLCvsTime[iieta][iiphi][iisign]->SetMarkerColor(4);
	graphLCvsTime[iieta][iiphi][iisign]->SetMarkerStyle(20);
	...
	for(int iiJob=0;iiJob<NJOBS;iiJob++){
	  LCvsTime[iieta][iiphi][iisign][iiJob]=0.;
	}
	delete graphLCvsTime[iieta][iiphi][iisign];
      }
    }
  }
